...
* 系统的外观、行为和操作应该保持一致性；
* 交互必须由用户控制；
* 尽可能保证用户在与界面交互时的效率，采用用户熟悉的概念与语言；
* 提供简单的操作方式，界面设计采取简洁明了的风格，扁平化设计；
2. 快捷键设计
统一采用操作系统内置的快捷键设计，不额外添加多余的快捷键。
3. 错误信息显示标准；
对于错误信息显示，统一采用弹出警告对话框进行错误消息提醒。并且设置确认键来令用户确认已注意到错误消息
4. 屏幕布局的限制
由于影院系统是一个跨平台的系统，因此屏幕布局会受到分辨率差异的限制，因此必须为不同分辨率进行分组，并为每个分组设计合适的布局方案。
5. 标准按钮
* 用户的客户端的界面有弹出当前使用的用户信息的按钮，弹出的是包含有用户个人信息设置，应用设置，查询历史，订票历史，当前订票菜单选项的菜单；
* 用户的客户端的界面有搜索框；
* 用户的客户端的界面有有导航栏进行各个功能模块的跳转；
* 系统管理用户的客户端界面有导航栏，实现对多个功能模块内容、信息、数据的管理
* 系统管理用户的客户端界面有一个帮助按钮，用来查询影院系统内的相关操作。
* 系统管理用户的客户端界面有一个查看系统管理用户个人信息按钮，用来查看、修改个人信息；
* 新闻管理用户的客户端界面有一个导航栏，导向对新闻内容管理的不同操作；
* 新闻管理用户的客户端界面有一个帮助按钮，用来查询影院系统用于新闻管理的相关操作；
* 新闻管理用户的客户端界面有一个查看新闻管理用户个人信息按钮，用来查看、修改个人信息；
* 影片信息管理用户的客户端界面有一个导航栏，导向对上映影片管理的不同操作；
* 影片信息管理用户的客户端界面有一个帮助按钮，用来查询影院系统用于影片信息管理的相关操作；
* 影片信息管理用户的客户端界面有一个查看影片信息管理个人信息按钮，用来查看、修改个人信息；
* 财务管理用户的客户端界面有一个导航栏，导向对产生的订单交易管理的不同操作；
* 财务管理用户的客户端界面有一个帮助按钮，用来查询影院系统用于查看交易、管理订单的相关操作；
* 财务管理用户的客户端界面有一个查看影片信息管理个人信息按钮，用来查看、修改个人信息；

### 硬件接口

影院系统主要程序以及核心代码都运行在服务器上，因此，在服务器上应该保留服务器与多平台进行数据交换的接口，采用TCP/IP协议进行通信。

### 软件接口

* 影院系统需要连接MySQL数据库，因此要设计与MySQL数据库的接口，用来从数据库当中读取数据以及向数据库中写入数据，目的是保存、查看过往数据。
* 影院系统需要设计接口接入第三方支付平台，使用第三方支付平台的支付功能，交换的是订票购票的订单信息，满足用户日常在线支付习惯，为用户提供合适的支付方式。

### 通信接口

* 在PC端设备上，通过WEB浏览器进行浏览数据、提交信息；
* 在移动设备上，通过应用将数据按TCP/IP协议要求封装，通过网络发送到服务器端，服务器端按同样协议发送反馈，本地配置数据仅保存在本地的存储器上；
